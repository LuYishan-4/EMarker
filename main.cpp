...
int main(int argc, char** argv) {
    ...
    if (!calibration_path.empty()) {
        ...
        calibration =
            tel::calibration::load_calibration(
                calibration_path
            );
    ...
    cv::Matx33d camera_matrix =
        tel::config::kFallbackCameraMatrix;
    ...
    tel::pipeline::CMarkerPipeline pipeline(
        camera_matrix,
        dist_coeffs
    );
    ...
    double best_green_error =
        std::numeric_limits<double>::max();

    double best_pnp_error =
        std::numeric_limits<double>::max();

    double best_homography_error =
        std::numeric_limits<double>::max();
    ...
    auto process_frame =
        ...
        for (const auto& marker : markers) {
            ...
            best_pnp_error =
                std::min(
                    best_pnp_error,
                    marker.pnp_reprojection_rms()
                );

            best_homography_error =
                std::min(
                    best_homography_error,
                    marker.homography_reprojection_rms()
                );
        }
        ...
        draw_text(
            display,
            "A/D or Arrow: previous/next frame",
            3
        );
        ...
        if (green_result.has_value()) {
            ...
            for (std::size_t i = 0;
                ...
                std::cout
                    << "  Green[" << i << "]\n"
                    << "    center        : ("
                    << green_result->centers[i].x
                    << ", "
                    << green_result->centers[i].y
                    << ")\n"
                    << "    area          : "
                    << green_result->areas[i]
                    << '\n'
                    << "    area_ratio    : "
                    ...
                    << "    geometry      : "
                    << (
                        green_result->area_ratios[i] >= 0.7f
                            ? "PASS"
                            : "FAIL"
                    )
                    << '\n';
