...
#include <opencv2/geometry.hpp> // OpenCV 5 將 contourArea/arcLength/approxPolyDP 移到此處
#endif
...
// ------------------------------------------------------------
constexpr std::array<int, kN> kContourToTemplate = {0, 2, 3, 1, 6, 4, 5, 7};
...
std::array<cv::Point3f, kN> buildCModel3D() {
  ...
  for (int i = 0; i < kN; ++i) {
    model[i] = cv::Point3f(static_cast<float>(model2d[i].x),
                           static_cast<float>(model2d[i].y), 0.0f);
  }
  return model;
}
...
std::array<cv::Point2f, kN>
reorderPolygon(const std::vector<cv::Point> &polygon, int offset,
               bool reverse) {
  std::array<cv::Point2f, kN> result{};
  for (int i = 0; i < kN; ++i) {
    int index = reverse ? (offset - i + kN * 2) % kN : (offset + i) % kN;
    result[i] = cv::Point2f(static_cast<float>(polygon[index].x),
...
bool projectHomographyPoint(const cv::Mat &H, const cv::Point2d &src,
                            cv::Point2f &dst) {
  ...
  if (std::abs(w) < kEpsilon)
    return false;
  const double u =
      (H.at<double>(0, 0) * x + H.at<double>(0, 1) * y + H.at<double>(0, 2)) /
      w;
  const double v =
      (H.at<double>(1, 0) * x + H.at<double>(1, 1) * y + H.at<double>(1, 2)) /
      w;
...
HomographyResult
calculateHomography(const std::array<cv::Point2f, kN> &imagePoints) {
  ...
  if (!projectHomographyPoint(result.H, greenMm, result.expectedGreen))
    return result;
...
PnPResult calculatePnP(const std::array<cv::Point2f, kN> &imagePoints) {
  ...
  const std::vector<cv::Point3f> objectPoints(modelArray.begin(),
                                              modelArray.end());
  ...
  for (int i = 0; i < 3; ++i)
    if (!std::isfinite(result.rvec[i]) || !std::isfinite(result.tvec[i]))
      return result;
  if (result.tvec[2] <= 0.0)
    return result;

  std::vector<cv::Point2f> projected;
  try {
    cv::projectPoints(objectPoints, result.rvec, result.tvec, cameraMatrix,
  ...
  try {
    cv::projectPoints(greenObject, result.rvec, result.tvec, cameraMatrix,
                      distCoeffs, greenProjected);
  } catch (const cv::Exception &) {
    return result;
  }
...
std::vector<BlueCandidate> find_blue_candidates(const cv::Mat &blue_mask) {
  ...
  cv::findContours(blue_mask, contours, cv::RETR_EXTERNAL,
  ...
  for (std::size_t ci = 0; ci < contours.size(); ++ci) {
    ...
    if (area < config::kMinBlueArea)
      continue;
    if (contour.size() < config::kMinContourPoints)
      continue;
    ...
    cv::approxPolyDP(contour, polygon, config::kApproxEpsilonRatio * perimeter,
                     true);
    ...
    const double greenDelta = cv::norm(geometry.homography.expectedGreen -
    ...
    candidate.valid = candidate.homography_valid && candidate.pnp_valid &&
                      std::isfinite(greenDelta) &&
                      greenDelta <= config::kMaxGreenPredictionError;

    if (kDebugBlue) {
      std::cout << "  BEST rev=" << geometry.reverse
                << " off=" << geometry.offset
